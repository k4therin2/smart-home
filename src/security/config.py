...
SLACK_SERVER_HEALTH_WEBHOOK = os.getenv("SLACK_SERVER_HEALTH_WEBHOOK")  # #colby-server-health: Weekly reports
...
UFW_BLOCK_THRESHOLD = int(os.getenv("UFW_BLOCK_THRESHOLD", "10"))  # Alert after N blocks from same IP in 5 min
SUDO_FAILED_THRESHOLD = int(os.getenv("SUDO_FAILED_THRESHOLD", "3"))  # Alert after N failed sudo in 10 min
DISK_SPACE_THRESHOLD = int(os.getenv("DISK_SPACE_THRESHOLD", "85"))  # Alert when disk > N% full
COST_VELOCITY_THRESHOLD = float(os.getenv("COST_VELOCITY_THRESHOLD", "1.00"))  # Alert if >$N spent in 1 hour
